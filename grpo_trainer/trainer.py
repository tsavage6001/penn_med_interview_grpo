...
class GRPOTrainer:
    ...
    def train_step(
        ...
        policy_loss = -(ratios * advantages.unsqueeze(1)).mean()
    ...
    def _prepare_training_data(
        ...
        for level in range(1, self.config.num_turns, 2):
            for i in range(0, len(branches), self.config.branches**level):
                ...
                for branch in sibling_group:
                    if len(branch) > level:
                        turn = branch[level]
                        print(f"Appending turn at level {level}: {turn.text[:30]}...")
        ...
        for idx, branch in enumerate(branches):
            print(f"Branch {idx} length: {len(branch)}")
        print(f"all_queries length: {len(all_queries)}")
        ...
        all_queries = [torch.tensor(q, dtype=torch.long) for q in all_queries]
        all_responses = [torch.tensor(r, dtype=torch.long) for r in all_responses]
        
        # Find the maximum length across both queries and responses
        max_len = min(
            self.config.max_length,
            max(
                max(q.size(0) for q in all_queries),
                max(r.size(0) for r in all_responses)
            )
        )
        
        def pad_to_max(seq, max_len, pad_token_id):
            # Truncate if necessary
            if seq.size(0) > max_len:
                seq = seq[:max_len]
            pad_len = max_len - seq.size(0)
            if pad_len > 0:
                return torch.cat([seq, torch.full((pad_len,), pad_token_id, dtype=seq.dtype, device=seq.device)])
            return seq
        
        queries = torch.stack([pad_to_max(q, max_len, pad_token_id) for q in all_queries]).to(self.config.device)
        responses = torch.stack([pad_to_max(r, max_len, pad_token_id) for r in all_responses]).to(self.config.device)
